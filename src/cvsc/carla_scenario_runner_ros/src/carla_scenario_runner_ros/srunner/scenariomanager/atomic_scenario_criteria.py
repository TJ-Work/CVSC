...
class InRouteTest(Criterion):
    ...
    def __init__(self, actor, radius, route, offroad_max, name="InRouteTest", terminate_on_failure=False):
        ...
        self.error_array = deque(maxlen=20000)
        self.DISTANCE_THRESHOLD = radius
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            if off_route:
                ...
                self.error_array.append(shortest_distance)
    ...
    def terminate(self, new_status):
        print "mean squared error: {}".format(
            str(np.mean(np.square(self.error_array)))
        )

        super(InRouteTest, self).terminate(new_status)
...
class RouteCompletionTest(Criterion):
    ...
    def __init__(self, actor, route, name="RouteCompletionTest", terminate_on_failure=False, DISTANCE_THRESHOLD=15.0):
        """
        """
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            if self._percentage_route_completed > 99.0 and location.distance(self.target) < self.DISTANCE_THRESHOLD:
                ...
                print(location.distance(self.target), location, self.target)
                self.list_traffic_events.append(route_completion_event)
                self.test_status = "SUCCESS"

...
class PublishGlobalPlan(AtomicBehavior):

    def __init__(self, name, route_array):
        self.route_array = route_array
        self.publisher = rospy.Publisher("/carla/ego_vehicle/waypoints", Path, queue_size=1, latch=True)
        super(PublishGlobalPlan, self).__init__(name)

    def setup(self, unused_timeout=15):
        this_msg = Path()
        this_msg.header.frame_id = "map"
        for point in self.route_array:
            location = point[0]
            pose = PoseStamped()
            ...
            this_msg.poses.append(pose)
        self.publisher.publish(this_msg)
        print("latch_global_plan")
        return True
    
    def initialise(self):
        print("run behavior {} intialise function".format(self.name))

    def update(self):
        new_status = py_trees.common.Status.SUCCESS
        print("run behavior {} update function".format(self.name))
        return new_status
    
    def terminate(self, new_status):
        print("run behavior {} terminate function".format(self.name))
